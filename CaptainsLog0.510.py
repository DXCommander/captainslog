...
class CaptainsLog:
    def __init__(self, root):
        self.root = root
        self.root.title("CaptainsLog V0.500")
        self.qsos = []
    ...
    def setup_ui(self):
        style = ttk.Style()
        style.configure("Treeview", font=("Arial", 10))  # Proportional font
        ...
        self.tree.heading("snt", text="SNT", anchor="center")
        self.tree.heading("rcv", text="RCV", anchor="center")
        self.tree.heading("freq", text="Freq", anchor="center")
        self.tree.heading("mode", text="Mode", anchor="center")
        self.tree.heading("name", text="Name", anchor="w")
        self.tree.heading("comments", text="Comments", anchor="w")


        self.tree.column("callsign", width=100, anchor="w", stretch=False)
        self.tree.column("snt", width=40, anchor="center", stretch=False)
        self.tree.column("rcv", width=40, anchor="center", stretch=False)
        self.tree.column("freq", width=80, anchor="center", stretch=False)
        self.tree.column("mode", width=60, anchor="center", stretch=False)
        self.tree.column("name", width=140, anchor="w", stretch=True)
        self.tree.column("comments", width=260, anchor="w", stretch=True)
    ...
    def log_qso(self, event=None):
        ...
        if callsign and callsign != "CALL":
            ...
            # Optional: update partial_check.json
            if name_val:
                try:
    ...
    def next_focus(self, event):
        ...
        return "break"
    def format_qso_display(self, qso):
        return f"{qso['callsign']:<12}{qso['snt']:<5}{qso['rcv']:<5}{qso['freq']:<9}{qso['name']:<22}{qso['comments']}"
        ...
        if os.path.exists(self.partial_check_path):
            try:
                ...
                if callsign in partial_db:
                    ...
                    if self.name_entry.is_placeholder_active():
                        self.name_entry.delete(0, 'end')
                        self.name_entry.insert(0, known.get("name", ""))
                        self.name_entry['fg'] = self.name_entry.default_fg_color
                        Tooltip(self.name_entry, "Auto-filled from Partial Check")
                    if self.comments_entry.is_placeholder_active() and known.get("comments"):
                        ...
                        Tooltip(self.comments_entry, "Auto-filled from Partial Check")
                else:
                    pass
            except Exception:
                pass
