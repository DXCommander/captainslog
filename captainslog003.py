...
class CaptainsLog:
    def show_help(self):
        ...
        tk.Label(help_win, text="CaptainsLog V0.3", font=("Arial", 14, "bold")).pack(pady=(10, 5))
        tk.Label(help_win, text="Created by Callum M0MCX (DXCommander)", font=("Arial", 10)).pack()
        tk.Label(help_win, text="Powered by Python + ChatGPT", font=("Arial", 10)).pack()

        # Scrollable text area for help info
        ...
        help_text.insert("1.0",
            ...
            "- Marking Left or Rejoined does nothing to database\n\n"
            "- Add your call, Freq and Mode at top\n"
            "- Enter calls at bottom - use Space Bar or ENter\n"
            "- Database re-written each time you make QSO\n\n"
            "GitHub page:\nhttps://github.com/DXCommander/captainslog/releases\n\n"
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("CaptainsLog V0.3")
        self.qsos = []
    ...
    def setup_ui(self):
        ...
        self.tree.tag_configure("left", foreground="#777777")
    ...
    def log_qso(self, event=None):
        ...
        if callsign and callsign != "CALL":
            ...
            display = f"{callsign:<12}{qso['snt']:<5}{qso['rcv']:<5}{qso['name']:<22}{qso['comments']}"
    ...
    def show_context_menu(self, event):
        ...
        self.context_menu.delete(1, 'end')
        item_tags = self.tree.item(row_id, "tags")
        if "left" in item_tags:
            self.context_menu.add_command(label="Mark Rejoined", command=self.mark_rejoined)
        else:
            self.context_menu.add_command(label="Mark LEFT", command=self.mark_left)
    ...
    def edit_entry(self):
        ...
        if selected_item:
            ...
            fields = ["callsign", "snt", "rcv", "name", "comments"]
            entries = {}
            ...
            def save_edit():
                ...
                display = f"{updated['callsign']:<12}{updated['snt']:<5}{updated['rcv']:<5}{updated['name']:<22}{updated['comments']}"
                self.tree.item(selected_item, text=display)
                self.save_current_log()
                edit_win.destroy()
    ...
    def export_adif(self):
        ...
        with open(adif_file, "w") as f:
            ...
            for qso in self.qsos:
                ...
                f.write(line)
        if messagebox.askyesno("Export", "ADIF export complete.\nOpen in Notepad?"):
    ...
    def load_previous_log(self):
        if os.path.exists(self.log_file):
            answer = messagebox.askyesno("Previous Log Found", "Continue previous log?")
            if not answer:
            ...
            try:
                with open(self.log_file, 'r') as f:
                    ...
                    for qso in self.qsos:
                        display = f"{qso['callsign']:<12}{qso['snt']:<5}{qso['rcv']:<5}{qso['name']:<22}{qso['comments']}"
                        self.tree.insert("", "end", text=display)
                self.callsign_entry.focus_force()
